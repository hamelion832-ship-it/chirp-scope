...
export function ChartPanel({ title, data, xLabel, yLabel, color }: ChartPanelProps) {
  ...
  // Auto-scale: compute domain from data
  ...
  return (
    <div className="chart-panel flex flex-col h-full">
      <h3 className="text-xs font-mono font-semibold text-signal-green glow-green mb-2 truncate">{title}</h3>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 5, right: 10, left: -10, bottom: 20 }}>
            ...
            <XAxis
              ...
              label={{ value: xLabel, position: "bottom", offset: 5, style: { fontSize: 9, fill: "hsl(215 15% 50%)" } }}
            />
            <YAxis
              stroke="hsl(215 15% 50%)"
              tick={{ fontSize: 9, fill: "hsl(215 15% 50%)" }}
