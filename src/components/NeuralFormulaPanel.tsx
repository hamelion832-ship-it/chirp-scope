...
export function NeuralFormulaPanel() {
  ...
  const computeSignalStats = useCallback((stored: StoredSignal, samples: SignalSample[]) => {
    ...
    return {
      sf: stored.sf, bw: stored.bw, duration: stored.duration, nSymbols: stored.n_symbols,
      sampleCount: samples.length, meanAmp: mean, maxAmp: max, stdAmp: std,
      zeroCrossings, trend,
    };
  }, []);
  ...
  const handleAISuggest = useCallback(async () => {
    if (selected.length === 0) {
      toast.error("Выберите хотя бы один сигнал");
      return;
    }
    setSuggesting(true);
    ...
    try {
      ...
      toast.error(`Ошибка ИИ: ${msg}`);
    } finally {
      setSuggesting(false);
    }
  }, [selected, signals, buildSamples, computeSignalStats]);

  const applyAISuggestion = useCallback(() => {
    if (aiSuggestion) {
      setFormulaType(aiSuggestion.formulaType);
      toast.info(`Выбрана формула: ${FORMULA_TYPES[aiSuggestion.formulaType].label}`);
    }
  }, [aiSuggestion]);
  ...
  const comparisonData = useMemo(() => {
    ...
    const predicted = generatePrediction(activeResult.coefficients, activeResult.formulaType, tMin, tMax, samples.length);
  ...
  const coeffComparison = useMemo(() => {
    ...
    return keys.map(k => {
      const values = entries.map(([id, r]) => ({
        id: id.slice(0, 6),
        value: (r.coefficients as unknown as Record<string, number>)[k] ?? 0,
      }));
      ...
      <div className="chart-panel space-y-3">
        ...
        <div className="flex flex-wrap gap-2 items-center border-t border-border pt-3">
          ...
          {(Object.keys(FORMULA_TYPES) as FormulaType[]).map(ft => (
            <button key={ft} onClick={() => { setFormulaType(ft); setAutoFit(false); }}
              className={`text-[10px] font-mono px-2.5 py-1 rounded border transition-colors ${
                !autoFit && formulaType === ft
                  ? 'bg-signal-cyan/20 text-signal-cyan border-signal-cyan/40'
                  : 'bg-secondary text-muted-foreground border-border hover:text-foreground'
              }`}>
              {FORMULA_TYPES[ft].label}
            </button>
          ))}
          <button onClick={() => setAutoFit(!autoFit)}
            ...
            <Zap className="w-3 h-3" /> Авто-подбор
          </button>
          ...
          <div className="bg-signal-magenta/10 border border-signal-magenta/30 rounded px-3 py-2 space-y-1.5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono text-muted-foreground">
                  (уверенность: {(aiSuggestion.confidence * 100).toFixed(0)}%)
                </span>
              </div>
              <button onClick={applyAISuggestion}
                className="text-[10px] font-mono bg-signal-magenta/20 text-signal-magenta px-2 py-0.5 rounded hover:bg-signal-magenta/30 transition-colors">
                Применить
              </button>
            </div>
            ...
            {aiSuggestion.alternativeType && (
              <p className="text-[10px] font-mono text-muted-foreground">
                Альтернатива: {FORMULA_TYPES[aiSuggestion.alternativeType].label}
              </p>
            )}
          </div>
        )}
          ...
          {signals.map(s => {
            ...
            return (
              <div key={s.id}
                ...
                {hasResult && r && (
                  <span className="text-[9px] text-signal-green">{FORMULA_TYPES[r.formulaType]?.label.slice(0, 3)}</span>
                )}
          ...
          {activeResult && activeStored ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div key={k} className="flex items-center justify-between text-[11px] font-mono">
                        ...
                        {editCoeffs ? (
                          ...
                          <span className="text-foreground">
                            {((activeResult.coefficients as unknown as Record<string, number>)[k] ?? 0).toFixed(6)}
                          </span>
                        )}
                      </div>
                    ))}
                ...
                <div className="chart-panel">
                  ...
                  <div className="space-y-1.5 text-[11px] font-mono">
                    ...
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Эпох:</span>
            ...
            <div className="chart-panel flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-[10px] font-mono text-muted-foreground mt-1">
                5 типов формул · Авто-подбор · ИИ рекомендации
              </p>
            </div>
          )}
        </div>
