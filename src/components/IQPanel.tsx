...
export function IQPanel({ points }: IQPanelProps) {
  ...
  return (
    <div className="chart-panel flex flex-col h-full">
      <h3 className="text-sm font-mono font-semibold text-signal-cyan glow-cyan mb-3">IQ диаграмма</h3>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 5, right: 10, left: 0, bottom: 20 }}>
            ...
            <XAxis
              ...
              label={{ value: "I (синфазная)", position: "bottom", offset: 5, style: { fontSize: 10, fill: "hsl(215 15% 50%)" } }}
            />
            <YAxis
              type="number"
              dataKey="y"
              ...
              stroke="hsl(215 15% 50%)"
              tick={{ fontSize: 10, fill: "hsl(215 15% 50%)" }}
              label={{ value: "Q (квадратурная)", angle: -90, position: "insideLeft", offset: 10, style: { fontSize: 10, fill: "hsl(215 15% 50%)" } }}
            />
            <Tooltip
              contentStyle={{ background: "hsl(220 18% 10%)", border: "1px solid hsl(220 15% 18%)", borderRadius: 6, fontSize: 11 }}
            />
            <Scatter data={scatterData} fill="hsl(185 90% 55%)" r={2} opacity={0.7} />
          </ScatterChart>
        </ResponsiveContainer>
      </div>
    </div>
